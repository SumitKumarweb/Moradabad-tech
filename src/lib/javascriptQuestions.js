 ...
 * Based on the JavaScript Questions README
 * Contains 155 output-based JavaScript questions
 */
...
console.log(y);`,
    ...
    correctAnswer: 2,
    explanation: "We can unpack values from arrays or properties from objects through destructuring. For example: `[a, b] = [1, 2];` The value of `a` is now `1`, and the value of `b` is now `2`. What we actually did in the question, is: `[y] = [1, 2, 3, 4, 5];` This means that the value of `y` is equal to the first value in the array, which is the number `1`. When we log `y`, `1` is returned."
  },
  {
    id: "js-q60",
    number: 60,
...
console.log(firstName);`,
    ...
    correctAnswer: 3,
    explanation: "By using destructuring assignment syntax we can unpack values from arrays, or properties from objects, into distinct variables: `const { firstName } = { firstName: 'Lydia' };` Also, a property can be unpacked from an object and assigned to a variable with a different name than the object property: `const { firstName: myName } = { firstName: 'Lydia' };` Therefore, `firstName` does not exist as a variable, thus attempting to access its value will raise a `ReferenceError`."
  },
  {
    id: "js-q77",
    number: 77,
...
console.log(typeof member);`,
    ...
    correctAnswer: 2,
    explanation: "Classes are syntactical sugar for function constructors. The equivalent of the `Person` class as a function constructor would be: `function Person(name) { this.name = name; }` Calling a function constructor with `new` results in the creation of an instance of `Person`, `typeof` keyword returns `\"object\"` for an instance. `typeof member` returns `\"object\"`."
  },
  {
    id: "js-q91",
    number: 91,
...
getItems(["banana", "apple"], "pear", "orange")`,
    ...
    correctAnswer: 3,
    explanation: "`...args` is a rest parameter. The rest parameter's value is an array containing all remaining arguments, **and can only be the last parameter**! In this example, the rest parameter was the second parameter. This is not possible, and will throw a syntax error. `function getItems(fruitList, favoriteFruit, ...args) { return [...fruitList, ...args, favoriteFruit]; } getItems(['banana', 'apple'], 'pear', 'orange');` The above example works. This returns the array `[ 'banana', 'apple', 'orange', 'pear' ]`"
  },
  {
    id: "js-q95",
    number: 95,
...
console.log(food);`,
    ...
    correctAnswer: 0,
    explanation: "We set the value of the `favoriteFood` property on the `info` object equal to the string with the pizza emoji, `'🍕'`. A string is a primitive data type. In JavaScript, primitive data types don't interact by reference. In JavaScript, primitive data types (everything that's not an object) interact by _value_. In this case, we set the value of the `favoriteFood` property on the `info` object equal to the value of the first element in the `food` array, the string with the pizza emoji in this case (`'🍕'`). A string is a primitive data type, and interact by value (see my blogpost if you're interested in learning more) Then, we change the value of the `favoriteFood` property on the `info` object. The `food` array hasn't changed, since the value of `favoriteFood` was merely a _copy_ of the value of the first element in the array, and doesn't have a reference to the same spot in memory as the element on `food[0]`. When we log food, it's still the original array, `['🍕', '🍫', '🥑', '🍔']`."
  },
  {
    id: "js-q110",
    number: 110,
    ...
    correctAnswer: 0,
    explanation: "With the `JSON.parse()` method, we can parse JSON string to a JavaScript value. Stringifying a number into valid JSON, then parsing the JSON string to a JavaScript value: `const jsonNumber = JSON.stringify(4); // '4' JSON.parse(jsonNumber); // 4` Stringifying an array value into valid JSON, then parsing the JSON string to a JavaScript value: `const jsonArray = JSON.stringify([1, 2, 3]); // '[1, 2, 3]' JSON.parse(jsonArray); // [1, 2, 3]` Stringifying an object  into valid JSON, then parsing the JSON string to a JavaScript value: `const jsonArray = JSON.stringify({ name: 'Lydia' }); // '{\"name\":\"Lydia\"}' JSON.parse(jsonArray); // { name: 'Lydia' }`"
  },
  {
    id: "js-q111",
    number: 111,
...
console.log(two.next().value);`,
    ...
    correctAnswer: 2,
    explanation: "With the `yield` keyword, we `yield` values in a generator function. With the `yield*` keyword, we can yield values from another generator function, or iterable object (for example an array). In `generatorOne`, we yield the entire array `['a', 'b', 'c']` using the `yield` keyword. The value of `value` property on the object returned by the `next` method on `one` (`one.next().value`) is equal to the entire array `['a', 'b', 'c']`. In `generatorTwo`, we use the `yield*` keyword. This means that the first yielded value of `two`, is equal to the first yielded value in the iterator. The iterator is the array `['a', 'b', 'c']`. The first yielded value is `a`, so the first time we call `two.next().value`, `a` is returned."
  },
  {
    id: "js-q113",
    number: 113,
...
export function getQuestionByNumber(number) {
  return javascriptQuestions.find(q => q.number === number);
}
...
export function getTotalQuestions() {
  return javascriptQuestions.length;
}
